...
for t in range(T):
    v_list,c_list = [],[]
    prev_probs = p0
    sampled_policies_index = np.random.choice(len(total_policies),size=n_pol)
    for pol_ind in sampled_policies_index:
        vf,cf = rsla.find_min_vf_cf(total_policies[pol_ind],us,R,C,init_state,dist)
        c_list.append(cf)
        v_list.append(vf)
        p0[pol_ind] = p0_hat[pol_ind]*np.exp(alpha*(vf+lambda_*cf))
    c_list = np.array(c_list)
    v_list = np.array(v_list)
    #lambda_ = np.min([np.max([lambda_hat+eta*(b-np.dot(prev_probs,c_list)),0]),zi])
    p0_hat[sampled_policies_index] = p0_hat[sampled_policies_index]*np.exp(alpha*(v_list + lambda_*c_list))
    #lambda_hat = np.min([np.max([lambda_+eta*(b-np.dot(p0,c_list)),0]),zi])
    cf_,vf_ = np.dot(p0[sampled_policies_index],c_list),np.dot(p0[sampled_policies_index],v_list)
    vf_list.append(vf_)
    cf_list.append(cf_)
data_dict={'vf':np.array(vf_list),'cf':np.array(cf_list)}
import pandas as pd
df = pd.DataFrame(data_dict)
df.to_excel('VF_CF_values_Sampled_RSLA_new_no_lambda'+env_type[env_chosen]+"_"+model_type[model_chosen]+".xlsx")
